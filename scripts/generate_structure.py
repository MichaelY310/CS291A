...
def regenerate_structure(bug):
    repo_name = bug["repo"]
    commit_id = bug["base_commit"]
    instance_id = bug["instance_id"]
    playground_folder = "/home/yuanyang/SURFI-p1/playground"
    structure_folder = "/home/yuanyang/SURFI-p1/structure"

    # Generate a temperary folder and add uuid to avoid collision
    playground_folder = os.path.join(playground_folder, f"{instance_id}")
    structure_dir = os.path.join(structure_folder, f"{instance_id}.jsonl")
    subprocess.run(["rm", "-rf", f"{structure_dir}"], check=True)
    ...
    if not os.path.exists(playground_folder):
        ...
        print("Checking out commit...")
        subprocess.run(["git", "-C", f"{playground_folder}", "checkout", commit_id], check=True)
    else:
        print("Repo clone found. Skip cloning repo.")
    print("Generating repo structure...")
    structure = create_structure(f"{playground_folder}")
    # subprocess.run(["rm", "-rf", f"{playground_folder}"], check=True)
    os.makedirs(os.path.dirname(structure_dir), exist_ok=True)
    with open(structure_dir, "w") as structure_file:
        json.dump(structure, structure_file)
...
if __name__ == "__main__":
    # Read instance IDs from good_structures.txt
    structure_folder = "/home/yuanyang/SURFI-p1/structure"
    with open("/home/yuanyang/CS291A/good_structures.txt", "r") as f:
        instance_ids = [line.strip() for line in f.readlines()]

    # Load the dataset to get bug information
    benchmark = "full"
    split = "train"
    if benchmark == "lite":
    ...
    for instance_id in tqdm(instance_ids):
        ...
        # Skip if structure file already exists
        if os.path.exists(structure_dir):
            print(f"Structure file for {instance_id} already exists. Skipping...")
            continue
            
        # Find the corresponding bug data
        bug_data = None
        for bug in swe_bench_data:
            if bug["instance_id"] == instance_id:
                bug_data = bug
                break
                
        if bug_data is None:
            print(f"Could not find bug data for instance_id {instance_id}")
            continue
            
        # Generate structure
        print(f"Generating structure for instance_id {instance_id}")
        regenerate_structure(bug_data)
